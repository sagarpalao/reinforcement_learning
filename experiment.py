...
if __name__ == "__main__": 
    ...
    #             p = Process(target=learn_and_plot, args=('LunarLander-v2', 'reinforce', [env.observation_space.shape[0], [hidden_units], env.action_space.n, alpha_policy, alpha_value], filename, 1))
    #             processes.append(p)
    #             p.start()
    # for p in processes:
    #     p.join()
    ...
    #             p = Process(target=learn_and_plot, args=('LunarLander-v2', 'ac', [env.observation_space.shape[0], [hidden_units], env.action_space.n, alpha_policy, alpha_value], filename, 1))
    #             processes.append(p)
    #             p.start()
    # for p in processes:
    #     p.join()
    ...
    #         for hidden_units in [10]:
    #             env = GridWorld687()
    #             filename = f"Figures/ac_gridworld_{str(alpha_policy).replace('.', '#')}_{str(alpha_value).replace('.', '#')}_{hidden_units}"
    #             p = Process(target=learn_and_plot, args=('gridworld', 'ac', [10, [hidden_units], 4, alpha_policy, alpha_value], filename, 0.9))
    #             processes.append(p)
    #             p.start()
    # for p in processes:
    #     p.join()
    ...
    #                 p = Process(target=learn_and_plot, args=('LunarLander-v2', 'sarsa', [env.observation_space.shape[0], [hidden_units], env.action_space.n, alpha_value, N, epsilon], filename, 1))
    #                 processes.append(p)
    #                 p.start()
    # for p in processes:
    #     p.join()
